...
애자일은 만드는 비용을 최적화합니다 — 짧은 스프린트와 작은 증분은 구현이
느리고 비싸던 시절의 장치입니다. AI 에이전트는 그 비용을 없애버렸고, 남은
비용은 검증되지 않은 가정 위에 쌓아 올리는 것입니다: 에이전트는 빠르고
자신만만하며, 망한 접근법도 테스트가 초록이 될 때까지 기꺼이 다듬습니다.
그래서 여기서 진행의 단위는 출하된 증분이 아니라 소거된 리스크이고, 비용은
노출을 줄일 때만 의미가 있으며, 게이트는 `stop`을 결론 내릴 수 있습니다.
